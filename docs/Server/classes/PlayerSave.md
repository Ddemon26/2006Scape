# PlayerSave

Package `com.rs2.game.players`.

Defined in [`2006Scape Server/src/main/java/com/rs2/game/players/PlayerSave.java`](2006Scape Server/src/main/java/com/rs2/game/players/PlayerSave.java).

game.entities.Player Save helper class.

```java
public class PlayerSave {
public static int loadGame(Client player, String playerName, String playerPass)
public static int loadPlayerInfo(Client player, String playerName, String playerPass, boolean doRealLogin)
public static String passwordHash(String token2)
public static boolean saveGame(game.entities.Player player)
```
