# BankPin

Package `com.rs2.game.content`.

Defined in [`2006Scape Server/src/main/java/com/rs2/game/content/BankPin.java`](2006Scape Server/src/main/java/com/rs2/game/content/BankPin.java).

Handles the BankPin on Bank's  @author Michael @author Ian / Core @author Linus @author Genesis

```java
public class BankPin {
public BankPin(game.entities.Player player)
public int dateExpired()
public int dateRequested()
public void closeBankPin()
public void pinSettingFrames()
public void bankPinSettings()
public boolean resetBankPin()
public void bankPinEnter(int button)
public void openPin()
public String toString()
```
