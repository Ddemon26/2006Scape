# Telekinetic

Package `com.rs2.game.content.minigames.magetrainingarena`.

Defined in [`2006Scape Server/src/main/java/com/rs2/game/content/minigames/magetrainingarena/Telekinetic.java`](2006Scape Server/src/main/java/com/rs2/game/content/minigames/magetrainingarena/Telekinetic.java).

Telekinetic helper class.

```java
public class Telekinetic {
public static Maze getMaze(int x, int y, int h)
public Point calcDirection(game.entities.Player player)
public Point getNewPos(int curX, int curY, int dirX, int dirY)
public Telekinetic(game.entities.Player c)
public void moveStatue(int itemX, int itemY)
public void execute(CycleEventContainer container)
public void stop()
public void goToMaze()
public void execute(CycleEventContainer container)
public void stop() {}
public void observeStatue(int itemX, int itemY)
public void resetStatue(int itemX, int itemY)
public static void process()
public static void updateInterface(game.entities.Player player)
```
