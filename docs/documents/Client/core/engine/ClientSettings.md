# core.engine.ClientSettings

Defined in [`2006Scape Client/src/main/java/core.engine.ClientSettings.java`](2006Scape Client/src/main/java/core.engine.ClientSettings.java).

core.engine.ClientSettings  @author Andrew (I'm A Boss on Rune-Server and Mr Extremez on Mopar & Runelocus)

```java
public class core.engine.ClientSettings {
public static final BigInteger RSA_MODULUS = new BigInteger("");
public static final BigInteger RSA_EXPONENT = new BigInteger("65537");
```
