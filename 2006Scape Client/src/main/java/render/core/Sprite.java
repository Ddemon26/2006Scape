package render.core; // Decompiled by Jad v1.5.8f. Copyright 2001 Pavel Kouznetsov.
...
public final class Sprite extends DrawingArea {
  ...
  public void crop() {
    ...
    for (int j = 0; j < height; j++) {
      for (int k = 0; k < width; k++)
        ai[(j + offsetY) * trimWidth + (k + offsetX)] = pixels[j * width + k];
    }
    pixels = ai;
    width = trimWidth;
    height = trimHeight;
  ...
  private void blendPixels(
      int i, int j, int ai[], int ai1[], int l, int i1, int j1, int k1, int l1) {
    int k; // was parameter
    int j2 = 256 - k1;
    for (int k2 = -i1; k2 < 0; k2++) {
      for (int l2 = -j; l2 < 0; l2++) {
        ...
        if (k != 0) {
          int i3 = ai[l1];
          ai[l1++] =
              ((k & 0xff00ff) * k1 + (i3 & 0xff00ff) * j2 & 0xff00ff00)
                      + ((k & 0xff00) * k1 + (i3 & 0xff00) * j2 & 0xff0000)
                  >> 8;
        } else {
          l1++;
        }
      ...
      for (j1 = 0; j1 < i; j1++) {
        ...
        for (k1 = -ai[j1]; k1 < 0; k1++) {
          if (ClientSettings.BILINEAR_MINIMAP_FILTERING) {
            ...
            int red =
                (sampleColor1 >> 16 & 0xff) * sampleAlpha1
                        + (sampleColor2 >> 16 & 0xff) * sampleAlpha2
                        + (sampleColor3 >> 16 & 0xff) * sampleAlpha3
                        + (sampleColor4 >> 16 & 0xff) * sampleAlpha4
                    & 0xff0000;
            int green =
                (sampleColor1 >> 8 & 0xff) * sampleAlpha1
                            + (sampleColor2 >> 8 & 0xff) * sampleAlpha2
                            + (sampleColor3 >> 8 & 0xff) * sampleAlpha3
                            + (sampleColor4 >> 8 & 0xff) * sampleAlpha4
                        >> 8
                    & 0xff00;
            int blue =
                (sampleColor1 & 0xff) * sampleAlpha1
                        + (sampleColor2 & 0xff) * sampleAlpha2
                        + (sampleColor3 & 0xff) * sampleAlpha3
                        + (sampleColor4 & 0xff) * sampleAlpha4
                    >> 16;
  ...
  private void copyMasked(
      int ai[], int i, byte abyte0[], int j, int ai1[], int k, int l, int i1, int j1, int k1) {
    int l1 = -(i >> 2);
    i = -(i & 3);
