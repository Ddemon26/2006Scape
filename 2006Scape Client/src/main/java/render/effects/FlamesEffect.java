...
/**
 * Handles the flame background effect shown on the login screen. Extracted from {@link Game}.
 *
 * <p>NOTE: Only names/constants added/renamed. **No logic has been changed.**
 */
public final class FlamesEffect {
  ...
  public void calcFlamesPosition() {
    ...
    for (int y = 1; y < grid - 1; y++) {
      for (int x = 1; x < 127; x++) {
        int pos = x + (y << 7);
        int value =
            game.flameBuffer2[pos + WIDTH_128]
                - game.flameGradient1[pos + game.flameOffset & game.flameGradient1.length - 1]
                    / GRADIENT_DIVISOR;
        if (value < 0) {
          value = 0;
        }
    ...
    game.flameLineOffsets[grid - 1] =
        (int)
            (Math.sin((double) game.loopCycle / 14D) * 16D
  ...
  public void doFlamesDrawing() {
    ...
    if (game.flameMainColor > 0) {
      for (int i = 0; i < COLOR_MAX; i++) {
        if (game.flameMainColor > 768) {
          game.flameBuffer[i] =
              game.blendColors(
                  game.flamePaletteRed[i],
                  game.flamePaletteGreen[i],
                  COLOR_BLEND_MAX - game.flameMainColor);
        } else if (game.flameMainColor > 256) {
          game.flameBuffer[i] = game.flamePaletteGreen[i];
        } else {
          game.flameBuffer[i] =
              game.blendColors(
                  game.flamePaletteGreen[i], game.flamePaletteRed[i], 256 - game.flameMainColor);
        }
      }
      ...
      for (int i = 0; i < COLOR_MAX; i++) {
        if (game.flameSecondaryColor > 768) {
          game.flameBuffer[i] =
              game.blendColors(
                  game.flamePaletteRed[i],
                  game.flamePaletteBlue[i],
                  COLOR_BLEND_MAX - game.flameSecondaryColor);
    ...
    System.arraycopy(
        game.titleBackgroundLeft.pixels, 0, game.titleLeftProducer.pixels, 0, IMAGE_COPY_COUNT);
    ...
    for (int y = 1; y < grid - 1; y++) {
      ...
      for (int x = startX; x < WIDTH_128; x++) {
        ...
        if (intensity != 0) {
          ...
          game.titleLeftProducer.pixels[dstIdx++] =
              ((color & 0xff00ff) * alpha + (bg & 0xff00ff) * invAlpha & 0xff00ff00)
                      + ((color & 0xff00) * alpha + (bg & 0xff00) * invAlpha & 0xff0000)
                  >> 8;
        } else {
          dstIdx++;
        }
    ...
    System.arraycopy(
        game.titleBackgroundRight.pixels, 0, game.titleRightProducer.pixels, 0, IMAGE_COPY_COUNT);
    ...
    for (int y = 1; y < grid - 1; y++) {
      ...
      for (int x = 0; x < width; x++) {
        ...
        if (intensity != 0) {
          ...
          game.titleRightProducer.pixels[dstIdx++] =
              ((color & 0xff00ff) * alpha + (bg & 0xff00ff) * invAlpha & 0xff00ff00)
                      + ((color & 0xff00) * alpha + (bg & 0xff00) * invAlpha & 0xff0000)
                  >> 8;
        } else {
