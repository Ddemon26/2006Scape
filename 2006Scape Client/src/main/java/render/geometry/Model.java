package render.geometry; // Decompiled by Jad v1.5.8f. Copyright 2001 Pavel Kouznetsov.

// Jad home page: http://www.kpdus.com/jad.html
// Decompiler options: packimports(3)

import core.engine.Game;
import core.managers.OnDemandFetcherParent;
...
public final class Model extends Animable {
    ...
    for (int i1 = 0; i1 < i; i1++) {
      ...
      if (model_1 != null) {
        ...
        for (int l1 = 0; l1 < model_1.texturedTriangleCount; l1++) {
          texTriangleX[texturedTriangleCount] =
              getOrCreateVertex(model_1, model_1.texTriangleX[l1]);
          texTriangleY[texturedTriangleCount] =
              getOrCreateVertex(model_1, model_1.texTriangleY[l1]);
          texTriangleZ[texturedTriangleCount] =
              getOrCreateVertex(model_1, model_1.texTriangleZ[l1]);
  ...
  public void calculateBounds() {
    ...
    diagonal2D =
        (int)
            (Math.sqrt(boundingRadius * boundingRadius + super.modelHeight * super.modelHeight)
                + 0.98999999999999999D);
    diagonal3D =
        diagonal2D
            + (int)
                (Math.sqrt(boundingRadius * boundingRadius + maxY * maxY) + 0.98999999999999999D);
  }
  ...
  private void calculateExtremes() {
    ...
    diagonal2D =
        (int) Math.sqrt(boundingRadius * boundingRadius + super.modelHeight * super.modelHeight);
  ...
  public void applyFrame(int i) {
    ...
    for (int k = 0; k < animFrame.transformationCount; k++) {
      ...
      transformVertices(
          frameBase.transformationType[l],
          frameBase.transformationList[l],
          animFrame.transformX[k],
          animFrame.transformY[k],
          animFrame.transformZ[k]);
    }
  }
  ...
  public void applyFrames(int ai[], int j, int k) {
    ...
    for (int j1 = 0; j1 < animFrame.transformationCount; j1++) {
      ...
      if (k1 != i1 || frameBase.transformationType[k1] == 0) {
        transformVertices(
            frameBase.transformationType[k1],
            frameBase.transformationList[k1],
            animFrame.transformX[j1],
            animFrame.transformY[j1],
            animFrame.transformZ[j1]);
      }
    }
    ...
    for (int l1 = 0; l1 < secondaryFrame.transformationCount; l1++) {
      ...
      if (i2 == i1 || frameBase.transformationType[i2] == 0) {
        transformVertices(
            frameBase.transformationType[i2],
            frameBase.transformationList[i2],
            secondaryFrame.transformX[l1],
            secondaryFrame.transformY[l1],
            secondaryFrame.transformZ[l1]);
      }
    }
  }
  ...
  public void applyLighting(int i, int j, int k, int l, int i1, boolean flag) {
    ...
    for (int i2 = 0; i2 < faceCount; i2++) {
      ...
      int j5;
      for (j5 = j3 * j4 - i4 * k3;
          l4 > 8192 || i5 > 8192 || j5 > 8192 || l4 < -8192 || i5 < -8192 || j5 < -8192;
          j5 >>= 1) {
        l4 >>= 1;
        i5 >>= 1;
      }
  ...
  public void applyShading(int i, int j, int k, int l, int i1) {
    for (int j1 = 0; j1 < faceCount; j1++) {
      ...
      if (faceRenderTypes == null) {
        ...
        int k2 =
            i
                + (k * vertexNormal.x + l * vertexNormal.y + i1 * vertexNormal.z)
                    / (j * vertexNormal.magnitude);
        ...
        int l2 =
            i
                + (k * vertexNormal1.x + l * vertexNormal1.y + i1 * vertexNormal1.z)
                    / (j * vertexNormal1.magnitude);
    ...
    for (int k = 0; k < faceCount; k++) {
      if (faceRenderTypes == null || faceRenderTypes[k] != -1) {
        ...
        if (flag && (i3 == -5000 || l3 == -5000 || k4 == -5000)) {
          ...
          if ((i3 - l3) * (projectedVertexY[j2] - projectedVertexY[k1])
                  - (projectedVertexY[l] - projectedVertexY[k1]) * (k4 - l3)
              > 0) {
    ...
    if ((j3 - j4) * (k7 - j7) - (i7 - j7) * (j5 - j4) > 0) {
      ...
      if (l == 4) {
        ...
        if (i8 == 0) {
          Texture.drawGouraudTriangle(i7, j7, k7, j3, j4, j5, HYPOT[0], HYPOT[1], HYPOT[2]);
          Texture.drawGouraudTriangle(
              i7, k7, COSINE[3], j3, j5, SINE[3], HYPOT[0], HYPOT[2], HYPOT[3]);
          return;
        }
        if (i8 == 1) {
