package util.compression; // Decompiled by Jad v1.5.8f. Copyright 2001 Pavel Kouznetsov.
...
public final class Decompressor {
  ...
  public Decompressor(
      RandomAccessFile randomaccessfile, RandomAccessFile randomaccessfile1, int j) {
    cacheIndex = j;
  ...
  public synchronized boolean writeEntry(int i, byte abyte0[], int j) {
    boolean flag = writeBlock(true, j, i, abyte0);
    if (!flag) flag = writeBlock(false, j, i, abyte0);
    return flag;
  }

  private synchronized boolean writeBlock(boolean flag, int j, int k, byte abyte0[]) {
    try {
      int l;
      if (flag) {
