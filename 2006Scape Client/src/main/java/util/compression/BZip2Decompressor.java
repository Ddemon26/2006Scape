...
public final class BZip2Decompressor {
  ...
  public static int decompress(
      byte[] outputBuffer, int outputLength, byte[] inputBuffer, int inputLength, int startIndex) {
    synchronized (state) {
  ...
  private static void decompressStream(BZip2State state) {
    ...
    while (flag19) {
      ...
      for (int l3 = 0; l3 < j4; l3++) {
        ...
        createHuffmanTables(
            state.limit[l3], state.base[l3], state.perm[l3], state.tempLen[l3], byte8, i, i4);
  ...
  private static void createHuffmanTables(
      int ai[], int ai1[], int ai2[], byte abyte0[], int i, int j, int k) {
    int l = 0;
