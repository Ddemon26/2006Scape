...
public final class QueuedMidiPlayer extends AbstractMidiController implements Runnable {
  ...
  public final synchronized void playMidi(int i, byte[] is, int i_6_, boolean bool) {
    midiFile.load(is);
  ...
  public final synchronized void poll(int i) {
    if (midiFile.isLoaded()) {
        ...
        while (i_15_ == midiFile.trackTicks[i_14_]) {
          ...
          if (midiFile.isTrackFinished()) {
            ...
            if (midiFile.allTracksFinished()) {
              if (strictMode) midiFile.resetTracks(l_16_);
              else {
                resetAllControllers((long) (int) (l_16_ / (long) ((midiFile.timeDivision) * 1000)));
                midiFile.clear();
                flushMessages();
                return;
              }
      ...
      if (i > -90) midiDevice = null;
      flushMessages();
    }
  }

  public final synchronized void stopMidi() {
    midiDevice.setActive(false);
    resetAllControllers((long) lastTimestamp);
    midiDevice.processBuffer(messageBuffer, bufferPos);
    bufferPos = 0;
  ...
  public final synchronized void adjustVolume(int i, int i_23_) {
