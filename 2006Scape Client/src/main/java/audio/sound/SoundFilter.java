...
final class SoundFilter {
  ...
  private float interpolateGain(int channel, int index, float f) {
    float f1 =
        gainCoefficients[channel][0][index]
            + f * (gainCoefficients[channel][1][index] - gainCoefficients[channel][0][index]);
    f1 *= 0.001525879F;
  ...
  private float interpolateFrequency(float f, int index, int channel) {
    float f1 =
        filterCoefficients[channel][0][index]
            + f * (filterCoefficients[channel][1][index] - filterCoefficients[channel][0][index]);
  ...
  public int compute(int channel, float f) {
    ...
    coefficients[channel][0] = -2F * f2 * (float) Math.cos(interpolateFrequency(f, 0, channel));
    coefficients[channel][1] = f2 * f2;
    for (int k = 1; k < filterPairs[channel]; k++) {
      float f3 = interpolateGain(channel, k, f);
      float f4 = -2F * f3 * (float) Math.cos(interpolateFrequency(f, k, channel));
      float f5 = f3 * f3;
      coefficients[channel][k * 2 + 1] = coefficients[channel][k * 2 - 1] * f5;
      coefficients[channel][k * 2] =
          coefficients[channel][k * 2 - 1] * f4 + coefficients[channel][k * 2 - 2] * f5;
      for (int j1 = k * 2 - 1; j1 >= 2; j1--) {
        coefficients[channel][j1] +=
            coefficients[channel][j1 - 1] * f4 + coefficients[channel][j1 - 2] * f5;
      }
