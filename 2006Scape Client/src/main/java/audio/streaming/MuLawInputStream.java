...
final class MuLawInputStream extends InputStream {
  ...
  public final synchronized int read(byte[] is, int i, int i_4_) {
    ...
    try {
      ...
      // core.engine.Game.method486(sampleBuffer, i_4_);//TODO incase
      for (int i_6_ = 0; i_6_ < i_4_; i_6_++) {
        int i_7_ = sampleBuffer[i_6_];
        if ((i_7_ + 8388608 & ~0xffffff) != 0) sampleBuffer[i_6_] = 0x7fffff ^ i_7_ >> 31;
      }
  ...
  private static final void encodeBuffer(
      byte[] is, int[] is_8_, byte[] is_9_, int i, int i_10_, int i_11_) {
    for (i = 0; i < i_11_; i++) is_9_[i_10_++] = is[(is_8_[i] >> 8) + 32768];
  }

  MuLawInputStream() {
    for (int i = -32768; i < 32768; i++) muLawTable[i + 32768] = encodeSample(i);
  }
}
