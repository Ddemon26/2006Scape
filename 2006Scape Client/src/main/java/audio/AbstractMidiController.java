...
public abstract class AbstractMidiController extends MidiPlayer {
  final void applyVolumeFade(int step, int volume, long timestamp) {
    ...
    if (volume != MusicSystem.midiVolume) {
      MusicSystem.midiVolume = volume;
  ...
  final boolean handleControlChange(int status, int controller, int value, long timestamp) {
    if ((status & 0xf0) == 176) {
      if (controller == 121) {
        ...
        MusicSystem.midiChannels[channel] = 12800;
        int scaled = calculateChannelVolume(channel);
      ...
      if (controller == 7 || controller == 39) {
        int channel = status & 0xf;
        if (controller == 7)
          MusicSystem.midiChannels[channel] =
              (MusicSystem.midiChannels[channel] & 0x7f) + (value << 7);
        else
          MusicSystem.midiChannels[channel] = (MusicSystem.midiChannels[channel] & 0x3f80) + value;
        int scaled = calculateChannelVolume(channel);
        sendShortMessage(status, 7, scaled >> 7, timestamp);
        sendShortMessage(status, 39, scaled & 0x7f, timestamp);
        return true;
  ...
  final void resetAllControllers(long timestamp) {
    ...
    for (int channel = 0; channel < 16; channel++) sendShortMessage(channel + 176, 0, 0, timestamp);
    for (int channel = 0; channel < 16; channel++)
      sendShortMessage(channel + 176, 32, 0, timestamp);
    for (int channel = 0; channel < 16; channel++) sendShortMessage(channel + 192, 0, 0, timestamp);
  }

  final void setMasterVolume(int volume, long timestamp) {
    MusicSystem.midiVolume = volume;
    for (int channel = 0; channel < 16; channel++) MusicSystem.midiChannels[channel] = 12800;
    for (int channel = 0; channel < 16; channel++) {
      int scaled = calculateChannelVolume(channel);
      sendShortMessage(channel + 176, 7, scaled >> 7, timestamp);
      sendShortMessage(channel + 176, 39, scaled & 0x7f, timestamp);
  ...
  private static final int calculateChannelVolume(int channel) {
    int value = MusicSystem.midiChannels[channel];
    value = (value * MusicSystem.midiVolume >> 8) * value;
