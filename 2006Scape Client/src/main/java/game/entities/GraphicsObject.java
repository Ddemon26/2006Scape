...
public final class GraphicsObject extends Animable {

  public GraphicsObject(
      int plane, int startCycle, int delay, int spotAnimId, int height, int y, int x) {
    finished = false;
  ...
  public Model getRotatedModel() {
    ...
    model_1.applyLighting(
        64 + spotAnimation.ambient, 850 + spotAnimation.contrast, -30, -50, -30, true);
    return model_1;
  }
  ...
  public void update(int elapsed) {
    for (frameCycle += elapsed; frameCycle > spotAnimation.animation.getFrameDelay(frame); ) {
      ...
      frame++;
      if (frame >= spotAnimation.animation.frameCount
          && (frame < 0 || frame >= spotAnimation.animation.frameCount)) {
        frame = 0;
        finished = true;
      }
    }
