...
public final class Player extends Entity {
  ...
  public Model getRotatedModel() {
    ...
    if (overlayModel != null) {
      ...
      if (Game.loopCycle >= animationStartCycle && Game.loopCycle < animationEndCycle) {
        ...
        model_1.translate(
            animationBaseX - super.x,
            animationBaseHeight - animationBaseY,
            animationBaseZ - super.y);
        ...
        model_1.translate(
            super.x - animationBaseX,
            animationBaseY - animationBaseHeight,
            super.y - animationBaseZ);
      }
    }
    model.pickable = true;
    return model;
  ...
  private Model getBaseModel() {
    ...
    if (model_1 == null) {
      ...
      for (int j3 = 0; j3 < 5; j3++) {
        if (bodyColors[j3] != 0) {
          model_1.recolor(
              Game.appearanceColorOptions[j3][0], Game.appearanceColorOptions[j3][bodyColors[j3]]);
          if (j3 == 1) {
            model_1.recolor(
                Game.additionalColorCodes[0], Game.additionalColorCodes[bodyColors[j3]]);
          }
        }
      }
  ...
  public Model getDialogueModel() {
    ...
    for (int j1 = 0; j1 < 5; j1++) {
      if (bodyColors[j1] != 0) {
        model.recolor(
            Game.appearanceColorOptions[j1][0], Game.appearanceColorOptions[j1][bodyColors[j1]]);
        if (j1 == 1) {
