...
final class ThreadedMidiPlayer extends MidiPlayerBase implements Runnable
{
    ...
    final synchronized void method827(int i, byte[] is, int i_6_,
				      boolean bool) {
		midiParser.method525(is);
		boolean bool_7_ = true;
		aBoolean1854 = bool;
		anInt1855 = 0;
		aRunnable_Impl1_1852.method12(false);
		method835(i_6_, i, (long) anInt1855);
		int i_8_ = midiParser.method533();
		for (int i_9_ = 0; i_9_ < i_8_; i_9_++) {
			midiParser.method526(i_9_);
			while (!midiParser.method521()) {
				midiParser.method520(i_9_);
				if (midiParser.anIntArray216[i_9_] != 0) {
					bool_7_ = false;
					break;
				}
				method847(i_6_ ^ 0x70, 0L, i_9_);
			}
		    midiParser.method522(i_9_);
		}
		if (bool_7_) {
			if (aBoolean1854)
				throw new RuntimeException();
			method838((long) anInt1855);
		    midiParser.method523();
		}
		method846();
    }
	...
	if (midiParser.method527()) {
	    int i_11_ = anInt1855;
	    int i_12_ = -200;
	    int i_13_ = aRunnable_Impl1_1852.method14(-29810);
	    long l = ((long) (i_11_ - (i_13_ + i_12_))
		      * (long) (midiParser.anInt213 * 1000));
	    for (;;) {
		int i_14_ = midiParser.method536();
		int i_15_ = midiParser.anIntArray216[i_14_];
		long l_16_ = midiParser.method532(i_15_);
		if (l < l_16_)
		    break;
		while (i_15_
		       == midiParser.anIntArray216[i_14_]) {
		    midiParser.method526(i_14_);
		    method847(126, l_16_, i_14_);
		    if (midiParser.method521()) {
			midiParser.method522(i_14_);
			if (midiParser.method531()) {
			    if (aBoolean1854)
				midiParser.method534(l_16_);
			    else {
				method838
				    ((long) (int) (l_16_
						   / (long) ((midiParser.anInt213)
							     * 1000)));
				midiParser.method523();
				method846();
				return;
    ...
    private final void method847(int i, long l, int i_17_) {
    	int i_18_ = midiParser.method529(i_17_);
    	if (i_18_ != 1) {
    		if ((i_18_ & 0x80) != 0) {
    			int i_19_  = (int) (l / (long) (midiParser.anInt213 * 1000));
    		...
    		midiParser.method528();
    }
    
    ThreadedMidiPlayer(Runnable_Impl1 runnable_impl1) {
