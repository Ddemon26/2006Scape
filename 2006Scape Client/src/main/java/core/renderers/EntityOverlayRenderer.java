...
public final class EntityOverlayRenderer {
  ...
  public void renderEntityOverlays() {
    renderGroundItemOverlays();
    renderEntityNames();
  }

  private void renderGroundItemOverlays() {
    for (int k5 = 0; k5 < 104; k5++) {
      for (int l5 = 0; l5 < 104; l5++) {
        NodeList itemList = game.groundArray[game.plane][k5][l5];
        if (itemList != null) {
          int offset = 5;
          for (Item item = (Item) itemList.reverseGetFirst();
            ...
            game.calcEntityScreenPos(k5 * 128 + 64, 20, l5 * 128 + 64);
            // only show ground items names if worth more than x (1k default)
            if (totalValue >= game.customSettingMinItemValue) {
              ...
              text += itemDef.name + " (" + GameUtils.intToShortLetter(totalValue) + " gp)";
              game.plainFont.textLeft(
                  color,
                  text,
                  game.spriteDrawY - offset,
                  game.spriteDrawX - (game.plainFont.getTextWidth(text) / 2));
              offset += 10;
            }
          }
        }
  ...
  private void renderEntityNames() {
    ...
    for (int j = -1; j < game.playerCount + game.npcCount; j++) {
      ...
      if (j < game.playerCount) {
        ...
        if (player.combatLevel == 0) {
          if (game.customSettingVisiblePlayerNames) {
            ...
            // ItemDef.getSprite(995, 1000, 0xffff00).drawTransparentSprite(game.spriteDrawX - 16,
            // game.spriteDrawY - l);
          ...
          if (player.privelage >= 1) {
            ...
            game.modIcons[icon].draw(
                game.spriteDrawX - player.name.length() * 3 - 16, game.spriteDrawY - 7);
          }
        }
        if (player.headIcon >= 0) {
          ...
          if (game.spriteDrawX > -1) {
            if (player.skullIcon < 2) {
              game.skullIcons[player.skullIcon].drawTransparentSprite(
                  game.spriteDrawX - 12, game.spriteDrawY - l);
              l += 25;
            }
            if (player.headIcon < 7) {
              game.headIcons[player.headIcon].drawTransparentSprite(
                  game.spriteDrawX - 12, game.spriteDrawY - l);
              l += 18;
            }
          }
        ...
        if (j >= 0 && game.hintIconState == 10 && game.selectedPlayerId == game.playerIndices[j]) {
          game.npcScreenPos(entity, entity.height + 15);
          if (game.spriteDrawX > -1) {
            game.headIconsHint[1].drawTransparentSprite(
                game.spriteDrawX - 12, game.spriteDrawY - l);
          }
        }
      } else {
        ...
        if (entityDef_1.headIcon >= 0 && entityDef_1.headIcon < game.headIcons.length) {
          game.npcScreenPos(entity, entity.height + 15);
          if (game.spriteDrawX > -1) {
            game.headIcons[entityDef_1.headIcon].drawTransparentSprite(
                game.spriteDrawX - 12, game.spriteDrawY - 30);
          }
        }
        if (game.hintIconState == 1
            && game.hintNpcIndex == game.npcIndices[j - game.playerCount]
            && game.loopCycle % 20 < 10) {
          game.npcScreenPos(entity, entity.height + 15);
          if (game.spriteDrawX > -1) {
            game.headIconsHint[0].drawTransparentSprite(
                game.spriteDrawX - 12, game.spriteDrawY - 28);
          }
        }
      }
      // Chat messages sent
      if (entity.textSpoken != null
          && (j >= game.playerCount
              ...
              || game.publicChatMode == 1 && game.isFriendOrSelf(((Player) entity).name))) {
        game.npcScreenPos(entity, entity.height);
        if (game.spriteDrawX > -1 && overheadTextCount < game.maxDisplayedText) {
          game.textWidth[overheadTextCount] =
              game.chatTextDrawingArea.measurePlainTextWidth(entity.textSpoken) / 2;
        ...
        try {
          ...
          if (game.spriteDrawX > -1) {
            ...
            DrawingArea.fillArea(
                5, game.spriteDrawY - 3, 0xff0000, 30 - i1, game.spriteDrawX - 15 + i1);
          }
        } catch (Exception e) {
        }
      }
      ...
      for (int j1 = 0; j1 < 4; j1++) {
        if (entity.hitsLoopCycle[j1] > game.loopCycle) {
          ...
          if (game.spriteDrawX > -1) {
            ...
            game.hitMarks[entity.hitMarkTypes[j1]].drawTransparentSprite(
                game.spriteDrawX - 12, game.spriteDrawY - 12);
            game.plainFont.textCenter(
                0, String.valueOf(entity.hitArray[j1]), game.spriteDrawY + 4, game.spriteDrawX);
            game.plainFont.textCenter(
                0xffffff,
                String.valueOf(entity.hitArray[j1]),
                game.spriteDrawY + 3,
                game.spriteDrawX - 1);
          }
        }
    ...
    for (int k = 0; k < overheadTextCount; k++) {
      ...
      while (flag) {
        ...
        for (int l2 = 0; l2 < k; l2++) {
          if (l1 + 2 > game.textY[l2] - game.textHeight[l2]
              && l1 - k2 < game.textY[l2] + 2
              && k1 - j2 < game.textX[l2] + game.textWidth[l2]
              && k1 + j2 > game.textX[l2] - game.textWidth[l2]
              && game.textY[l2] - game.textHeight[l2] < l1) {
      ...
      if (game.chatEffectsState == 0) {
        ...
        if (game.textEffects[k] == 1) {
          game.chatTextDrawingArea.drawWavyCenteredText(
              0, s, game.spriteDrawX, game.waveCycle, game.spriteDrawY + 1);
          game.chatTextDrawingArea.drawWavyCenteredText(
              i3, s, game.spriteDrawX, game.waveCycle, game.spriteDrawY);
        }
        if (game.textEffects[k] == 2) {
          game.chatTextDrawingArea.drawWavyText(
              game.spriteDrawX, s, game.waveCycle, game.spriteDrawY + 1, 0);
          game.chatTextDrawingArea.drawWavyText(
              game.spriteDrawX, s, game.waveCycle, game.spriteDrawY, i3);
        }
        if (game.textEffects[k] == 3) {
          game.chatTextDrawingArea.drawShakeText(
              150 - game.textCycles[k],
              s,
              game.waveCycle,
              game.spriteDrawY + 1,
              game.spriteDrawX,
              0);
          game.chatTextDrawingArea.drawShakeText(
              150 - game.textCycles[k], s, game.waveCycle, game.spriteDrawY, game.spriteDrawX, i3);
        }
        ...
        if (game.textEffects[k] == 5) {
          ...
          DrawingArea.setDrawingArea(
              ...
              0,
              512,
              game.spriteDrawY - game.chatTextDrawingArea.fontHeight - 1);
