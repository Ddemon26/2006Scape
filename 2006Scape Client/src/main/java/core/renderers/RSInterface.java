...
public final class RSInterface {
  ...
  public static void unpack(
      StreamLoader streamLoader, TextDrawingArea textDrawingAreas[], StreamLoader streamLoader_1) {
    ...
    while (stream.currentOffset < stream.buffer.length) {
          ...
          if (k3 == 1) {
            ...
            if (streamLoader_1 != null && s1.length() > 0) {
              ...
              rsInterface.sprites[j2] =
                  loadSprite(
                      Integer.parseInt(s1.substring(i5 + 1)), streamLoader_1, s1.substring(0, i5));
            }
          }
        }
      ...
      if (rsInterface.type == 4) {
        rsInterface.disabledText =
            stream.readString().replaceAll("RuneScape", ClientSettings.SERVER_NAME);
      ...
      if (rsInterface.type == 5) {
        ...
        if (streamLoader_1 != null && s.length() > 0) {
          ...
          rsInterface.sprite1 =
              loadSprite(Integer.parseInt(s.substring(i4 + 1)), streamLoader_1, s.substring(0, i4));
        }
        s = stream.readString();
        if (streamLoader_1 != null && s.length() > 0) {
          int j4 = s.lastIndexOf(",");
          rsInterface.sprite2 =
              loadSprite(Integer.parseInt(s.substring(j4 + 1)), streamLoader_1, s.substring(0, j4));
        }
      }
  ...
  private Model getModelForMedia(int i, int j) {
    ...
    if (i == 1) model = Model.create(j);
    if (i == 2) model = EntityDef.forID(j).getModel();
    if (i == 3) model = Game.myPlayer.getDialogueModel();
    if (i == 4) model = ItemDef.lookup(j).getInterfaceModel(50);
    if (i == 5) model = null;
    if (model != null) modelCache.put(model, (i << 16) + j);
    return model;
  }
  ...
  public static void discardInterface(int i) {
    ...
    for (int j = 0; j < interfaceCache.length; j++)
      if (interfaceCache[j] != null
          ...
          && interfaceCache[j].type != 2) interfaceCache[j] = null;
  }
  ...
  public Model prepareModel(int j, int k, boolean flag) {
    ...
    Model model_1 =
        new Model(true, AnimFrame.isNullFrame(k) & AnimFrame.isNullFrame(j), false, model);
    if (k != -1 || j != -1) {
  ...
  /** game.animation.Animation cycle counter. */
