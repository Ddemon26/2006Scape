...
public final class SceneRenderer {
    ...
    public void addPlayersToScene(boolean flag) {
        ...
        for (int l = 0; l < j; l++) {
            ...
            player.skipAnimations = (game.lowMem && game.playerCount > 50 || game.playerCount > 200) && !flag && player.currentAnimation == player.standAnimation;
            int j1 = player.x >> 7;
            int k1 = player.y >> 7;
            if (j1 < 0 || j1 >= 104 || k1 < 0 || k1 >= 104) {
                continue;
            }
            if (player.overlayModel != null && game.loopCycle >= player.animationStartCycle && game.loopCycle < player.animationEndCycle) {
                player.skipAnimations = false;
                player.animationBaseY = game.getTileHeight(game.plane, player.y, player.x);
                game.worldController.addAnimatingObject(game.plane, player.y, player, player.currentHeading, player.boundingBoxMaxY, player.x, player.animationBaseY, player.boundingBoxMinX, player.boundingBoxMaxX, i1, player.boundingBoxMinY);
                continue;
            }
            ...
            player.animationBaseY = game.getTileHeight(game.plane, player.y, player.x);
            game.worldController.addAnimableObject(game.plane, player.currentHeading, player.animationBaseY, i1, player.y, 60, player.x, player, player.forcedAnimation);
        }
    }
    ...
    public void addNpcsToScene(boolean flag) {
        for (int j = 0; j < game.npcCount; j++) {
            ...
            game.worldController.addAnimableObject(game.plane, npc.currentHeading, game.getTileHeight(game.plane, npc.y, npc.x), k, npc.y, (npc.size - 1) * 64 + 60, npc.x, npc, npc.forcedAnimation);
        }
    }
}