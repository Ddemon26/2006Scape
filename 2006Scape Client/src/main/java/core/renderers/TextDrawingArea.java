package core.renderers; // Decompiled by Jad v1.5.8f. Copyright 2001 Pavel Kouznetsov.
...
public final class TextDrawingArea extends DrawingArea {
  ...
  public void textLeft(int i, String s, int j, int l) {
    ...
    for (int i1 = 0; i1 < s.length(); i1++) {
      ...
      if (c != ' ') {
        renderGlyph(
            glyphPixels[c], l + xOffsets[c], j + yOffsets[c], glyphWidths[c], glyphHeights[c], i);
      }
      l += glyphAdvances[c];
    }
  }
    ...
    for (int i1 = 0; i1 < s.length(); i1++) {
      ...
      if (c != ' ') {
        renderGlyph(
            glyphPixels[c],
            j + xOffsets[c],
            l + yOffsets[c] + (int) (Math.sin(i1 / 2D + k / 5D) * 5D),
            glyphWidths[c],
            glyphHeights[c],
            i);
      }
  ...
  public void drawWavyText(int i, String s, int j, int k, int l) {
    ...
    for (int i1 = 0; i1 < s.length(); i1++) {
      ...
      if (c != ' ') {
        renderGlyph(
            glyphPixels[c],
            i + xOffsets[c] + (int) (Math.sin(i1 / 5D + j / 5D) * 5D),
            k + yOffsets[c] + (int) (Math.sin(i1 / 3D + j / 5D) * 5D),
            glyphWidths[c],
            glyphHeights[c],
            l);
      }
    ...
    for (int k1 = 0; k1 < s.length(); k1++) {
      ...
      if (c != ' ') {
        renderGlyph(
            glyphPixels[c],
            l + xOffsets[c],
            k + yOffsets[c] + (int) (Math.sin(k1 / 1.5D + j) * d),
            glyphWidths[c],
            glyphHeights[c],
            i1);
      }
  ...
  public void textLeftShadow(boolean _shadow, int _x, int _color, String s, int _y) {
    ...
    if (strikethrough) {
      DrawingArea.drawHorizontalLine(
          _y + (int) (fontHeight * 0.69999999999999996D), 0x800000, _x - l, l);
    }
  }
