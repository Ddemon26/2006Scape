...
public final class PendingSpawnManager {
  ...
  public void locatePendingSpawns() {
    ...
    for (;
        pendingSpawn != null;
        pendingSpawn = (PendingSpawn) game.pendingSpawns.reverseGetNext()) {
  ...
  public void locateSceneObject(PendingSpawn pendingSpawn) {
    ...
    if (pendingSpawn.category == 0) {
      i =
          game.worldController.getBoundaryObjectUid(
              pendingSpawn.plane, pendingSpawn.x, pendingSpawn.y);
    }
    if (pendingSpawn.category == 1) {
      i =
          game.worldController.getWallDecorationUid(
              pendingSpawn.plane, pendingSpawn.x, pendingSpawn.y);
    }
    if (pendingSpawn.category == 2) {
      i =
          game.worldController.getSceneObjectUid(
              pendingSpawn.plane, pendingSpawn.x, pendingSpawn.y);
    }
    if (pendingSpawn.category == 3) {
      i =
          game.worldController.getTileDecorationUid(
              pendingSpawn.plane, pendingSpawn.x, pendingSpawn.y);
    }
    if (i != 0) {
      int i1 =
          game.worldController.getObjectConfig(
              pendingSpawn.plane, pendingSpawn.x, pendingSpawn.y, i);
      j = i >> 14 & 0x7fff;
      k = i1 & 0x1f;
      l = i1 >> 6;
    }
  ...
  public void queuePendingSpawn(
    ...
    for (PendingSpawn pendingSpawnIter = (PendingSpawn) game.pendingSpawns.reverseGetFirst();
        ...
        pendingSpawnIter = (PendingSpawn) game.pendingSpawns.reverseGetNext()) {
