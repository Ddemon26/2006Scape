...
public final class OnDemandFetcher extends OnDemandFetcherParent implements Runnable {
  ...
  private void readData() {
    try {
      ...
      if (expectedSize == 0 && available >= 6) {
        ...
        current = null;
        for (OnDemandData onDemandData = (OnDemandData) requested.reverseGetFirst();
            onDemandData != null;
      ...
      if (expectedSize > 0 && available >= expectedSize) {
        ...
        if (expectedSize + completedSize >= abyte0.length && current != null) {
          if (clientInstance.decompressors[0] != null) {
            clientInstance.decompressors[current.type + 1].writeEntry(
  ...
  private void closeRequest(OnDemandData onDemandData) {
    try {
      if (socket == null) {
        ...
        openSocketTime = l;
        socket =
            clientInstance.openSocket(
                (ClientSettings.SINGLE_ONDEMAND || ClientSettings.SERVER_WORLD == 1)
                    ? 43594
  ...
  public void run() {
    try {
      while (running) {
        ...
        if (!flag) {
          for (OnDemandData onDemandData_1 = (OnDemandData) requested.reverseGetFirst();
              onDemandData_1 != null;
        ...
        if (clientInstance.loggedIn
            && socket != null
            && outputStream != null
            && (currentPriority > 0 || clientInstance.decompressors[0] == null)) {
          writeLoopCycle++;
  ...
  public OnDemandData getNextNode() {
    ...
    try {
      GZIPInputStream gzipinputstream =
          new GZIPInputStream(new ByteArrayInputStream(onDemandData.data));
      do {
  ...
  private void checkReceived() {
    ...
    while (onDemandData != null) {
      ...
      if (!crcMatches(
          versions[onDemandData.type][onDemandData.id],
          crcs[onDemandData.type][onDemandData.id],
          abyte0)) {
        abyte0 = null;
      }
      synchronized (pendingRequestQueue) {
