...
import core.handlers.SettingApplier;
...
import core.world.GroundItemSpawner;
import core.world.MapRegionBuilder;
import core.world.Pathfinder;
import core.world.PendingSpawn;
import core.world.WorldController;
import entities.EntityTextUpdater;
import entities.NpcUpdater;
import entities.PlayerStatsCalculator;
import entities.PlayerUpdater;
...
import util.compression.Decompressor;
...
public class Game extends RSApplet {
  ...
  public int getTileHeight(int plane, int worldY, int worldX) {
    ...
    int[][][] convertedTileFlags =
        new int[tileFlags.length][tileFlags[0].length][tileFlags[0][0].length];
    ...
    do {
      ...
      do {
        ...
        if (onDemandData.type == 2 && onDemandData.id == nextSong && onDemandData.data != null) {
          ...
          System.arraycopy(
              onDemandData.data, 0, MusicSystem.musicData, 0, MusicSystem.musicData.length);
  ...
  public void mainGameProcessor() {
    ...
    if (activeInterfaceType != 0) {
      ...
      if (super.clickMode2 == 0) {
        ...
        } else if ((oneMouseButtonMode == 1 || menuManager.menuHasAddFriend(menuActionRow - 1))
            && menuActionRow > 2) {
          menuManager.determineMenuSize();
  ...
  public void doAction(int i) {
    ...
    if (l == 606) {
      ...
      if (j2 != -1) {
        if (openInterfaceID == -1) {
          ...
          pushMessage(
              "Please close the public interface you have open before using 'report abuse'", 0, "");
        }
      }
    }
    if (l == 491) {
    ...
    if (l == 1125) {
      ...
      if (itemDef.description != null) {
        pushMessage(
            new String(itemDef.description)
                + " ("
                + GameUtils.intToKOrMil(itemDef.value)
                + "gp ea)",
            0,
            "");
      } else {
    ...
    if (l == 1448) {
      ...
      if (itemDef_1.description != null) {
        s6 =
            new String(itemDef_1.description)
                + " ("
                + GameUtils.intToKOrMil(itemDef_1.value)
                + "gp ea)";
      } else {
                ...
                if (k5 > DrawingArea.topX - 32
                  ...
                  if (itemSprite != null) {
                    ...
                    if (itemSprite.trimWidth == 33 || component.invStackSizes[i3] != 1) {
                      ...
                      plainFont.textLeft(
                          0, GameUtils.intToKOrMil(k10), j6 + 10 + j7, k5 + 1 + k6); // shadow
                      plainFont.textLeft(
                          0xffff00, GameUtils.intToKOrMil(k10), j6 + 9 + j7, k5 + k6); // top layer
                    }
                  }
                }
  ...
  public void updateOtherPlayers(Stream stream) {
    playerUpdater.updateOtherPlayers(stream);
  }
  ...
  public void handleMapPackets(Stream stream, int j) {
    mapPacketHandler.handleMapPackets(stream, j);
  }
  ...
  public boolean parsePacket() {
    ...
    try {
      ...
      if (pktType == 74) {
        ...
        } else GameMusicController.stopMusic(false);
        currentSong = i2;
        pktType = -1;
        return true;
      }
  ...
  public void renderGameView() {
    ...
    if (customSettingShowExperiencePerHour) {
      ...
      chatTextDrawingArea.textRightShadow(
          ...
          debugX + debugWidth - 4,
          Color.YELLOW.hashCode(),
          GameUtils.intToShortLetter((int) expPerHour),
          debugY);
      chatTextDrawingArea.textLeftShadow(
  ...
  public Game() {
    ...
    mapPacketHandler = new MapPacketHandler(this);
  ...
  public final PlayerUpdater playerUpdater;
  public final MapPacketHandler mapPacketHandler;
  ...
  void mouseWheelDragged(int i, int j) {
    InputHandler.handleMouseWheelDragged(this, i, j);
  }
