...
public final class CollisionMap {
  ...
  public boolean canReachArea(int i, int j, int k, int l, int i1, int j1, int k1) {
    ...
    if (k == j - 1
        && k1 >= i
        && k1 <= i2
        && (clippingFlags[k - xInset][k1 - yInset] & 8) == 0
        && (i1 & 8) == 0) {
      return true;
    }
    if (k == l1 + 1
        && k1 >= i
        && k1 <= i2
        && (clippingFlags[k - xInset][k1 - yInset] & 0x80) == 0
        && (i1 & 2) == 0) {
      return true;
    }
    return k1 == i - 1
            && k >= j
            && k <= l1
            && (clippingFlags[k - xInset][k1 - yInset] & 2) == 0
            && (i1 & 4) == 0
        || k1 == i2 + 1
            && k >= j
            && k <= l1
            && (clippingFlags[k - xInset][k1 - yInset] & 0x20) == 0
            && (i1 & 1) == 0;
  }
