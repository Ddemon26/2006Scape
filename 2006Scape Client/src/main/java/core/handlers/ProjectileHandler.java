...
public final class ProjectileHandler {
  ...
  public void processProjectiles() {
    for (Projectile projectile = (Projectile) game.projectileList.reverseGetFirst();
        projectile != null;
        projectile = (Projectile) game.projectileList.reverseGetNext()) {
        ...
        if (projectile.targetIndex > 0) {
          ...
          if (npc != null && npc.x >= 0 && npc.x < 13312 && npc.y >= 0 && npc.y < 13312) {
            projectile.track(
                game.loopCycle,
                npc.y,
                game.getTileHeight(projectile.plane, npc.y, npc.x) - projectile.heightOffset,
                npc.x);
          }
        }
        if (projectile.targetIndex < 0) {
          ...
          if (player != null
            ...
            projectile.track(
                game.loopCycle,
                player.y,
                game.getTileHeight(projectile.plane, player.y, player.x) - projectile.heightOffset,
                player.x);
          }
        }
