...
public class ClientIntegrationTest {
    ...
    public void testRandomGeneratorWithCacheKeys() {
        for (int i = 0; i < 10; i++) {
            ...
            assertSame("Should be able to store and retrieve nodes with random keys", 
                node, retrievedNode);
        }
    }
    ...
    public void testItemPileWithAnimableObjects() {
        ...
        assertSame("ItemPile should hold animable references correctly", animable2, itemPile.secondItem);
        assertSame("ItemPile should hold animable references correctly", animable3, itemPile.thirdItem);
        
        assertTrue("ItemPile coordinates should be within expected range", 
            itemPile.x >= -1000 && itemPile.x < 1000);
        assertTrue("ItemPile coordinates should be within expected range", 
            itemPile.y >= -1000 && itemPile.y < 1000);
        assertTrue("ItemPile height should be within expected range", 
